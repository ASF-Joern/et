                    ...
                    <div class="panel-body">
                        <div class="col-lg-12" id="articleQuantity" data-value="{$articleQuantity}">
                            {foreach from=$articles key=name item=article}
                                <div class="col-lg-4">
                                    <button type="button" id="{$name}" class=" ring btn btn-danger" data-value="{count($article)}">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.importSelectedRing').click(function() {
                batchAjax("{url controller=AsfAfterbuy action=addArticles}", { ringName: String($('button.ring.is--active').text()).split(" ")[0].trim() });
            });
