{block name="frontend_detail_image"}
                        ...
                        {block name='frontend_detail_image_default_image_element'}
                            ...
                            <span class="image--element"
                                    {if $sArticle.image}
                                        ...
                                        data-img-small="{$sArticle.image.thumbnails[1].source}"
                                        ...
                                        <span class="image--media">
                                            {if isset($image.thumbnails)}
                                                {block name='frontend_detail_images_picture_element'}
                                                    <img srcset="{$image.thumbnails[1].sourceSet}" src="{$image.thumbnails[1].sourceSet}" alt="{$alt}" itemprop="image" />
                                                {/block}
                                            {else}
    ...
    {if $asfTemplateFile == "trauringe.tpl" || $asfTemplateFile == "verlobungsringe.tpl" || $asfTemplateFile == "memoire-ringe.tpl"}
        {include file="frontend/detail/ringarten/{$asfTemplateFile}" options=$customProducts.options}
    {/if}
