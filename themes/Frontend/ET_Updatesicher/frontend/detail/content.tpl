...
{block name='frontend_index_content_inner'}
    {* We need the main category name here for styling *}
    {assign var="breadcrumbName" value={$sBreadcrumb[0].name|lower}}
    <div class="content product--details {$breadcrumbName|replace:" ":""}" itemscope itemtype="http://schema.org/Product"{if !{config name=disableArticleNavigation}} data-product-navigation="{url module="widgets" controller="listing" action="productNavigation"}" data-category-id="{$sArticle.categoryID}" data-main-ordernumber="{$sArticle.mainVariantNumber}"{/if} data-ajax-wishlist="true" data-compare-ajax="true"{if $theme.ajaxVariantSwitch} data-ajax-variants-container="true"{/if}>
