...
{if $sBasketItem.customProductMode == $IS_CUSTOM_PRODUCT_MAIN}
    {block name="frontend_checkout_cart_item_details_inline_swag_custom_products_surcharges"}
        ...
        <div class="custom-products--basket-overview">
            ...
            {* Iteration to prepare the data if we must display Trauringe *}
            {assign var="cpValues" value=['Ringwahl' => '', 'Profil' => '', 'Steinbesatz' => '', 'Herrenring-Gravur' => '', 'Damenring-Gravur' => '']}
            {foreach $sBasketItem.custom_product_adds as $option}
                {if $option.name == "Ringwahl" || $option.name == "Profil" || $option.name == "Steinbesatz"}
                    {$cpValues[$option.name] = $option.values[0].name}
                {/if}
                {if $option.name == "Herrenring-Gravur" || $option.name == "Damenring-Gravur"}
                    {$cpValues[$option.name] = $option.selectedValue.0}
                {/if}
            {/foreach}
            ...
            {block name="frontend_checkout_cart_item_details_inline_swag_custom_products_surcharges_list"}
                {foreach $sBasketItem.custom_product_adds as $option}
                    ...
                    {if $option.name == "Ringwahl" || $option.name == "Legierung" || $option.name == "Profil" || $option.name == "Steinbesatz"
                    || $option.name == "Herrenring-Gravur" || $option.name == "Damenring-Gravur" || $option.name == "Schriftart"}
                        {continue}
                    {/if}

                    {if $cpValues["Ringwahl"] == "Damenring" && ($option.name == "Herrenring-Breite" || $option.name == "Herrenring-Stärke" || $option.name == "Herrenring-Größe")}
                        {continue}
                    {/if}

                    {if $cpValues["Ringwahl"] == "Herrenring" && ($option.name == "Damenring-Breite" || $option.name == "Damenring-Stärke" || $option.name == "Damenring-Größe")}
                        {continue}
                    {/if}

                    {if $option.name == "Damenring-Größe"}
                        <fieldset>
                            <legend>Damenring</legend>
                                <ul class="custom-product--overview-list">
                                <li class="custom-product--overview-list-item {if $cpValues.Profil == "Standard-Profil"}is--hidden{/if}">
                                    <span class="custom-product--overview-option-name">
                                        <span class="overview-option-name--name">Profil {$cpValues.Profil|substr:6:1}</span>
                                    </span>
                                </li>
                    {/if}
                    {if $option.name == "Herrenring-Größe"}
                        </ul>
                        <fieldset>
                        <legend>Herrenring</legend>
                        <ul class="custom-product--overview-list">
                        <li class="custom-product--overview-list-item {if $cpValues.Profil == "Standard-Profil"}is--hidden{/if}">
                            <span class="custom-product--overview-option-name">
                                <span class="overview-option-name--name">Profil {$cpValues.Profil|substr:6:1}</span>
                    ...
                    {block name="frontend_checkout_cart_item_details_inline_swag_custom_products_surcharges_list_item"}
                        <li class="custom-product--overview-list-item">
                            ...
                            {block name="frontend_checkout_cart_item_details_inline_swag_custom_products_surcharges_item_name"}
                                ...
                                <span class="custom-product--overview-option-name" {if !empty($cpValues.Ringwahl)}style="display:inline-block; width:20%"{/if}>
                                    <span class="overview-option-name--name">
                                        {if !empty($cpValues.Ringwahl) && $option.name !== "Notiz"}
                                            {assign var="optionName" value="-"|explode:$option.name}
                                            {$optionName.1}:
                                        {else}
                                            {$option.name}:
                                        {/if}
                    ...
                    {/block}
                    {if $option.name == "Damenring-Stärke"}
                        <li class="custom-product--overview-list-item">
                            <span class="custom-product--overview-option-name" {if !empty($cpValues.Ringwahl)}style="display:inline-block; width:20%"{/if}>
                                <span class="overview-option-name--name">
                                    {if $cpValues.Steinbesatz != "Ohne Stein/e"}
                                        {assign var="stones" value=" ("|explode:$sBasketItem.additional_details.attr6}
                                        {$stones = " x "|explode:$stones.1}
                            ...
                            <span class="custom-product--overview-option-value">{$cpValues.Steinbesatz}</span>
                        </li>
                        {if $cpValues['Damenring-Gravur']}
                            <li class="custom-product--overview-list-item">
                                <span class="custom-product--overview-option-name">
                                    <span class="overview-option-name--name">Gravur:</span>
                                </span>
                                <span class="custom-product--overview-option-value" style="font-family:ArialLaser">{$cpValues['Damenring-Gravur']}</span>
                            </li>
                        {/if}
                        </ul>
                        </fieldset>
                    {/if}
                    {if $option.name == "Herrenring-Stärke"}
                        {if $cpValues['Herrenring-Gravur']}
                            <li class="custom-product--overview-list-item">
                                <span class="custom-product--overview-option-name">
                                    <span class="overview-option-name--name">Gravur:</span>
                                </span>
                                <span class="custom-product--overview-option-value" style="font-family:ArialLaser">{$cpValues['Herrenring-Gravur']}</span>
                            </li>
                        {/if}
                        </ul>
                        </fieldset>
