                        ...
                        <li class="custom-product--overview-list-item {if $option.name === "Notiz"}is--hidden{/if}">
                            ...
                            {block name="frontend_checkout_cart_item_details_inline_swag_custom_products_surcharges_item_name"}
                                {if $option.name != "Notiz"}
                                    <span class="custom-product--overview-option-name" style="display:inline-block; width:32%">
                                    <span class="overview-option-name--name">
                                        {if $option.name === "Ring-Größe" || $option.name === "Ring-Gravur"}
                            ...
                            {block name="frontend_checkout_cart_item_details_inline_swag_custom_products_surcharges_item_value"}
                                ...
                                <span class="custom-product--overview-option-value" {if $option.name === "Ring-Gravur"}style="font-family:ArialLaser"{/if}>
                                    {if $option.could_contain_values}
                                        {foreach $option.values as $value}
                                            {if $value.path}<i class="icon--paperclip"></i>&nbsp;{/if}
                                            {if $option.name === "Profil"}
                                                {if $value.name === "Profil4"}
                                                    Flach
                                                {else}
                                                    Rund
                                                {/if}
                                            {else}
                                                {if $option.name === "Steinbesatz"}
                                                    {assign var="stones" value=" / "|explode:$value.name}
                                                    {$smarty.session[$sBasketItem.articleID]} x {$stones.0}
                                                {else}
                                                    {$value.name}
                                                {/if}
                                            {/if}
